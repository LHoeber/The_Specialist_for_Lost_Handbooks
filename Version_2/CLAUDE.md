# Version_2 — porting Version_1 (Pygame) to a browser-deployable game

Renamed from `Version_1_web` on 2026-09-05, once this became where all
active development happens (see "Assets are now self-contained" below and
the folder's own history for why).
...
Sprites: the same PNG files, referenced by relative path from HTML/JS
instead of through `ASSET_DIR`.

## Assets are now self-contained (2026-09-05)

`assets/` here is this folder's **own** copy of `Version_1/assets/`, not a
reference to it — copied once, then decoupled. `rendering/assets.js`'s
`ASSET_DIR` points at `assets/` (this folder), not `../Version_1/assets/`.
This was a deliberate fix, not the original plan: pointing at `Version_1`'s
copy meant there was only one place sprites actually lived, which sounds
convenient but risked exactly the opposite of what it looks like -- since
`Version_1` (Pygame) is frozen and not meant to change further, but its
`assets/` folder would still be the one edited day-to-day for this version,
raising exactly the "am I editing the right copy" confusion a shared folder
was supposed to avoid. Two independent copies with no ongoing sync needed
(because `Version_1` never changes again) is simpler than one shared folder
serving two purposes. **Edit sprites in `Version_2/assets/` from now on;
`Version_1/assets/` is a frozen snapshot, useful only if `Version_1`
(Pygame) itself is ever run again as a scratchpad.**
...
then open `http://localhost:8000/Version_2/index.html` (serving from the
repo root works fine even though `Version_2/` is now self-contained; you
could equally serve from inside `Version_2/` itself). A Claude Cowork/Code
session with browser-automation
