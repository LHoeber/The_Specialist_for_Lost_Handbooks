...
- `Version_1_web/` — the browser-deployable port of `Version_1`, needed because participant recruitment (Prolific / Mechanical Turk) requires a zero-install, browser-based experience. **Start here for anything related to running the actual study.** See `Version_1_web/CLAUDE.md` for the full deployment-strategy reasoning and the file-by-file porting plan.
